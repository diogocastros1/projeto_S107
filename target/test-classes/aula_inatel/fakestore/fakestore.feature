Feature: Testando recursos da fakestoreapi.com.
    ...
    Scenario: Deletando produto 2
        ...
        When method delete
        Then status 200
    Scenario: Buscando um produto da lista
        Given url url_base
        And path '/products/3'
        When method get
        Then status 200
    Scenario: Buscando as categorias
        Given url url_base
        And path '/products/categories'
        When method get
        Then status 200
    Scenario: Listando produtos por uma categoria especifica
        Given url url_base
        And path '/products/category/jewelery'
        When method get
        Then status 200
    Scenario: Listando carrinho de compra
        Given url url_base
        And path '/carts/6'
        When method get
        Then status 200
